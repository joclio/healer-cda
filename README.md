...
3. **Plan** — first visit auto-assigns; later visits restore your saved plan. **Auto-assign CDs** asks before replacing manual work. Tweak rows, set tank targets, then **Copy NSRT** (in-game) or **Copy Viserio** (paste into [Viserio Cooldowns](https://wowutils.com/viserio-cooldowns) CD → Import). **Download plan** saves this fight as JSON; **Paste plan** loads it on another browser and matches healer, tank, and utility names onto the roster there.
